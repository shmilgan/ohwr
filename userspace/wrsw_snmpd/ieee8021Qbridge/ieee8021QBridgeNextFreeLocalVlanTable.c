...
struct ieee8021QBridgeNextFreeLocalVlanTable_entry {
    /* Index values */
...
static struct ieee8021QBridgeNextFreeLocalVlanTable_entry *
ieee8021QBridgeNextFreeLocalVlanTable_createEntry( int dummy
                 , u_long  ieee8021QBridgeNextFreeLocalVlanComponentId
                ) {
    struct ieee8021QBridgeNextFreeLocalVlanTable_entry *entry;

    entry = SNMP_MALLOC_TYPEDEF(struct ieee8021QBridgeNextFreeLocalVlanTable_entry);
    if (!entry)
        return NULL;
...
static void
ieee8021QBridgeNextFreeLocalVlanTable_removeEntry(struct ieee8021QBridgeNextFreeLocalVlanTable_entry *entry) {
    if (!entry)
...
static struct ieee8021QBridgeNextFreeLocalVlanTable_entry *
ieee8021QBridgeNextFreeLocalVlanTable_get_entry( netsnmp_variable_list *indexes ) {
    struct ieee8021QBridgeNextFreeLocalVlanTable_entry *row = NULL;
...
static struct ieee8021QBridgeNextFreeLocalVlanTable_entry *
ieee8021QBridgeNextFreeLocalVlanTable_get_next_entry( netsnmp_handler_registration *reginfo,
...
ieee8021QBridgeNextFreeLocalVlanTable_handler(
    ...
    struct ieee8021QBridgeNextFreeLocalVlanTable_entry          *table_entry;
    ...
    case MODE_GET:
        for (request=requests; request; request=request->next) {
            ...
            table_entry =     ieee8021QBridgeNextFreeLocalVlanTable_get_entry( table_info->indexes );
    ...
    case MODE_GETNEXT:
        for (request=requests; request; request=request->next) {
            table_info  =     netsnmp_extract_table_info( request);
            table_entry =     ieee8021QBridgeNextFreeLocalVlanTable_get_next_entry( reginfo, request,
...
/** Initialize the ieee8021QBridgeNextFreeLocalVlanTable table by defining its contents and how it's structured */
static void
initialize_table_ieee8021QBridgeNextFreeLocalVlanTable(void)
{
    ...
    reg = netsnmp_create_handler_registration(
              "ieee8021QBridgeNextFreeLocalVlanTable",     ieee8021QBridgeNextFreeLocalVlanTable_handler,
    ...
    table_info->min_column = COLUMN_IEEE8021QBRIDGENEXTFREELOCALVLANINDEX;
    table_info->max_column = COLUMN_IEEE8021QBRIDGENEXTFREELOCALVLANINDEX;
...
void
init_ieee8021QBridgeNextFreeLocalVlanTable(void)
{
