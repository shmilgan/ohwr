...
netsnmp_pdu *cli_snmp_getnext(oid _oid[MAX_OID_LEN])
{
    ...
    pdu->non_repeaters = 0;
    snmp_add_null_var(pdu, _oid, MAX_OID_LEN);
...
int cli_snmp_getnext_int(oid _oid[MAX_OID_LEN], int *oid_len)
{
    netsnmp_pdu *response;
    int err = 0, value = 0;
    netsnmp_variable_list *vars;

    response = cli_snmp_getnext(_oid);
    if (response) {
        vars = response->variables;
        memcpy(_oid, vars->name, vars->name_length);
...
char *cli_snmp_getnext_string(oid _oid[MAX_OID_LEN], int *oid_len)
{
    netsnmp_variable_list *vars;
    netsnmp_pdu *response;
    char *value = NULL;
...
void cli_snmp_set_int(oid _oid[MAX_OID_LEN], char *val, char type)
{
    ...
    if (snmp_add_var(pdu, _oid, MAX_OID_LEN, type, val)) {
        errno = -EINVAL;
        return;
    }
