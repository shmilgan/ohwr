...
typedef struct i2c_bus_t
{
	int 		type;
	void*		type_specific;
	int32_t		(*transfer)(struct i2c_bus_t* bus, uint32_t address,  uint32_t to_write, uint32_t to_read, uint8_t* data);
	int32_t		(*scan)(struct i2c_bus_t* bus, uint32_t address);
} i2c_bus_t;
