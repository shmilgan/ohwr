		...
		int ppshift_taps)
{
	...
	_fpga_writel(FPGA_BASE_GEN_10MHZ + GEN10_REG_PR, h_width);
	_fpga_writel(FPGA_BASE_GEN_10MHZ + GEN10_REG_DCR, l_width);
	_fpga_writel(FPGA_BASE_GEN_10MHZ + GEN10_REG_CSR, cshift_ns/CNT_RES);
	_fpga_writel(FPGA_BASE_GEN_10MHZ + GEN10_REG_IOR, sigdel_taps);
	_fpga_writel(FPGA_BASE_PPS_GEN + offsetof(struct PPSG_WB, DELAY), ppshift_taps);
	sleep(1);
	/* now read the actual delay (in taps) from IODelays */
	sigdel_taps = _fpga_readl(FPGA_BASE_GEN_10MHZ + GEN10_REG_IOR);
	sigdel_taps >>= GEN10_IOR_TAP_CUR_SHIFT;
	ppshift_taps = _fpga_readl(FPGA_BASE_PPS_GEN + offsetof(struct PPSG_WB, DELAY));
	ppshift_taps >>= PPSG_DELAY_TAP_CUR_SHIFT;
